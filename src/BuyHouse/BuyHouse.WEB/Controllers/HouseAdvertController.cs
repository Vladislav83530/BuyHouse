    ...
    public class HouseAdvertController : Controller
    {
        ...
        public async Task<IActionResult> GetHouseAdvert(int houseAdvertId)
        {
            ...
            try
            {
                ...
                GetHouseAdvertViewModel vm = new GetHouseAdvertViewModel
                {
                    HouseAdvert = houseAdvertModel,
        ...
        public async Task<IActionResult> Index(HouseAdvertFilter filter, int pageSize, int page = 1)
        {
            try
            {
                ResponseHouseAdvertDTO responseHouseAdvertDTO = await _houseAdvertFilterService
                    .GetHouseAdvertByParametersAsync(filter, pageSize, page);

                var houseAdvertShortModels = _mapper.Map<IEnumerable<HouseAdvert>, List<HouseAdvertShortModel>>(responseHouseAdvertDTO.HouseAdverts);

                IndexFilterViewModel vm = new IndexFilterViewModel()
                {
                    HouseAdverts = houseAdvertShortModels,
                    HouseAdvertFilter = filter,
